    ...
    try {
      ...
      if (result.status === "success") {
        ...
        setActiveData({
          ...
          // Aquí es donde se "guardan" en el estado local para mostrarse en la tabla
          packets: paquetes_pcap || [] 
        });

        setChatLog([{ sender: 'AI', text: guia_tutor.mensaje_inicial }]);
      }
    } catch (error) {
      ...
      <div className="flex-1 flex flex-col gap-6 min-h-0">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
          <div className="flex justify-between items-center px-1">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Entrenamiento Forense</span>
          </div>
            ...
            {Object.keys(levelMap).map(id => (
              <button 
                ...
                className={`px-3 py-3 border rounded-xl text-[10px] font-bold transition-all ${
                  currentLevel === id ? 'ring-2 ring-indigo-500 bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200 hover:bg-white'
                }`}
              >
                NIVEL {levelMap[id]}
              </button>
        ...
        <div className="flex-1 flex gap-6 overflow-hidden">
          <div className="flex-1 flex flex-col gap-4 overflow-hidden">
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
              <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
                <h3 className="text-xs font-bold text-gray-500 uppercase">Tráfico de Red Detectado</h3>
                    ...
                    {activeData?.packets?.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan="6" className="py-20 text-center text-slate-400 italic">
                          {isLoading ? "Consultando base de datos forense..." : "Selecciona un nivel del mapa para visualizar los paquetes."}
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="h-44 bg-white rounded-2xl shadow-lg border border-indigo-100 flex flex-col overflow-hidden">
                ...
                {chatLog.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.sender === 'User' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] p-2 rounded-xl text-xs ${
                      msg.sender === 'User' ? 'bg-slate-100 text-slate-800' : 'bg-indigo-600 text-white shadow-md'
                    }`}>
              ...
              <div className="p-2 bg-gray-50 border-t flex gap-2">
                ...
                />
                <button onClick={sendSimChat} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider">
                  Reportar
                </button>
              </div>
            </div>
          ...
          <div className="w-72 flex flex-col">
            <div className="flex-1 bg-slate-900 rounded-3xl shadow-xl p-5 text-white flex flex-col border border-slate-700 overflow-hidden">
              <h3 className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-4">Investigación Guiada</h3>
              <div className="flex-1 overflow-y-auto text-xs text-slate-300 leading-relaxed space-y-3">
                {!activeData ? (
                  <p className="text-slate-500 italic">Selecciona un nivel para recibir el briefing de la misión.</p>
                ) : (
                ...
                <div className="mt-4 pt-4 border-t border-slate-800">
                  <div className="flex flex-wrap gap-1">
                    {activeData.concepts.map((c, i) => (
                      <span key={i} className="bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-2 py-1 rounded-[4px] text-[8px] font-bold uppercase">
                        {c}
                      </span>
                    ))}
                  </div>
